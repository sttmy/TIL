...
class GoodsRead :
    ...
    def goodsFix(self):
        try:
            ...
            cursor = conn.cursor()
            in_code = int(input('수정할 상품 코드를 입력하세요: '))
            sql = f'select * from goods where code = {in_code}'
            cursor.execute(sql)
            rows = cursor.fetchall()            
            if len(rows) < 1:
                print('상품코드가 올바르게 입력되지 않았습니다.')
            else:
                print("------------ 수정할 상품 조회 --------------")
                for row in rows:
                    print(row)  
                in_put = input("위의 상품을 수정하시겠습니까? (Y/N) ")
                if in_put == 'Y':
                    in_name = input("새로운 상품명을 입력하세요: ")
                    in_su = int(input("수정할 수량을 입력하세요: "))
                    in_dan = int(input("수정할 단가를 입력하세요: "))
